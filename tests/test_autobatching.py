...
def test_chunking_auto_batcher(
    si_base_state: BaseState, fe_fcc_state: BaseState, lj_calculator
):
...
def test_chunking_auto_batcher_with_indices(
    ...
    batcher = ChunkingAutoBatcher(
        model=lj_calculator, states=states, metric="n_atoms", max_metric=100.0
    )
...
def test_determine_max_batch_size_fibonacci(
    si_base_state: BaseState, lj_calculator, monkeypatch
):
